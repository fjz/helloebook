...
import com.rockchip.utils.MyMath;
...
public class SinglePage extends View {
	...
	private void autoFlipPage(final float tx,final float ty,final boolean isSuccessFlip){
		...
		new Thread(){
			public void run(){
				int count = 0;
				boolean flag = false;
				MyMath line = MyMath.initLine(tapPoint.x, tapPoint.y, tx, ty);
				while(true){
					try {
						if(isLeftPage){
	...
	private void calculate(){
		...
		MyMath AP = MyMath.initLine(tapPoint.x, tapPoint.y, px, py);
		...
		MyMath BC = AP.getPBLine((tapPoint.x+px)/2, (tapPoint.y+py)/2);
		mXIntersectionPoint.x = BC.getXbyY(mXIntersectionPoint.y);
		R = MyMath.twoPointDistance(tapPoint, mXIntersectionPoint)/2;	
		MyMath PbPn = BC.LineTranslationWidthDistance((float)(R*Math.PI/2),direction);
		MyMath CnCb = BC.LineTranslationWidthDistance((float)(R*Math.PI/2-R), direction);
		MyMath AB = MyMath.initLine(tapPoint.x, tapPoint.y, mXIntersectionPoint.x, mXIntersectionPoint.y);
		mPnPoint = PbPn.getCross(AB);
		mPbPoint.x = PbPn.getXbyY(py);
		mPbPoint.y = py;
		if(direction){
		AP.change(mPbPoint.x, mPbPoint.y);
		MyMath Cn2Cb2 = CnCb.LineTranslationWidthDistance(30, false);
		mSdnPoint = AP.getCross(Cn2Cb2);
		}
		mPcnPoint = CnCb.getCross(AB);
		mPcbPoint.x = CnCb.getXbyY(py);
		mPcbPoint.y = py;
		mPcPoint = MyMath.getMidPoint(mPcnPoint, mPcbPoint);
		mYIntersectionPoint.y = BC.getYbyX(mYIntersectionPoint.x);
		MyMath AC = MyMath.initLine(tapPoint.x, tapPoint.y, mYIntersectionPoint.x, mYIntersectionPoint.y);
		...
		mPcb2Point.x = px;
		mPc2Point = MyMath.getMidPoint(mPcn2Point, mPcb2Point);
		...
		MyMath Cn2Cb2 = CnCb.LineTranslationWidthDistance(30, true);
		mSdnPoint = AP.getCross(Cn2Cb2);
		}
